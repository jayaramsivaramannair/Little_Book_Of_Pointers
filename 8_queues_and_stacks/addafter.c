...
void enqueue(LIST *queue, LISTITEM *item) {
	LISTITEM *temp;
...
LISTITEM *add_after(LIST *queue, int position, LISTITEM *item) {
	LISTITEM *temp;
	int i;
...
int main() {
	...
	for (int i = 0; i < 5; i++) {			// as before, populate the queue
		...
		enqueue(&list, temp);				// and put it in the queue
	}

	printf("the length of the queue is %d\n", queue_length(&list));
	// add item at a user entered index
	printf("enter the index of the queue entry ... ");
	requested_index = atoi(fgets(input, 50, stdin));
	temp = (LISTITEM*)malloc(sizeof(LISTITEM));
	// uncomment this (for debugging)
	// printf("address of new item = %p\n", temp);
	temp->data = requested_index + 10;			// set payload to a -ve number so we can see where it is when we print out the queue
	if (add_after(&list, requested_index, temp) == NULL) {	// if we can't do it we will get NULL returned
	...
	*/
	...
	do {							// keep going until the queue is empty
		temp = dequeue(&list);		// if the queue is empty we will get NULL returned
		if (temp != NULL) {
			printf("data: %d\n", temp->data);
			free(temp);				// call 'free' to tidy up 
		}
